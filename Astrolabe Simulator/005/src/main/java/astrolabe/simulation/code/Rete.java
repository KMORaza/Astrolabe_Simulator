...
public class Rete extends Group {
    ...
    private void draw() {
        ...
        for (Map.Entry<String, double[]> entry : stars.entrySet()) {
            ...
            // Apply atmospheric refraction for objects above horizon
            if (altAz[0] > -1) { // Only apply refraction if altitude > -1 degree
    ...
    private double applyAtmosphericRefraction(double apparentAltitudeDeg) {
        // Saemundsson's formula for atmospheric refraction
        // Input: apparent altitude in degrees (already refracted)
        // Output: true altitude in degrees

        // Convert to arcminutes
        double alt = apparentAltitudeDeg * 60;

        // Calculate refraction in arcminutes (R)
        double R = 1.02 / Math.tan(Math.toRadians((alt + 10.3 / (alt + 5.11)) / 60));

        // Convert refraction back to degrees and subtract from apparent altitude
        double trueAltitudeDeg = apparentAltitudeDeg - (R / 60);
    ...
    private void addPlanet(String name, double[] planetPos, double lst, double eps, double size, Color color, Node symbol) {
        ...
        // Apply atmospheric refraction for planets above horizon
        if (altAz[0] > -1) {
            altAz[0] = applyAtmosphericRefraction(altAz[0]);
        }
