...
public class Rete extends Group {
    ...
    private double calculateAngle(double x1, double y1, double x2, double y2) {
        // Convert screen coordinates to angular distance using stereographic projection
        double dx = x2 - x1;
        double dy = y2 - y1;
        double distance = Math.sqrt(dx*dx + dy*dy);

        // Convert to angular degrees (stereographic projection formula)
    ...
    private void draw() {
        ...
        // Re-add measurement tools first so they stay on top
